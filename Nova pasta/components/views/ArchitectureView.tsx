...
const ArchitectureView: React.FC = () => {
  ...
  useEffect(() => {
    const loadNodes = async () => {
      ...
      setLoadError(null);
      try {
        const data = await architectureService.listNodes();
        setNodes(data);
      } catch {
        setLoadError('Nao foi possivel carregar a visao de arquitetura.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return <LoadingSpinner text="Carregando arquitetura..." />;
  }
  ...
  const channelNode = nodesMap.CH;
  const coreNode = nodesMap.CORE;
  const moduleNode = nodesMap.MOD;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-slate-700 text-center">{channelNode?.label ?? 'Canal'}</h3>
          {channelNode?.children.map((node) => (
        ...
        <div className="space-y-4">
          <div className="text-center">
            <h3 className="text-xl font-semibold text-slate-700">{coreNode?.label ?? 'Nucleo'}</h3>
            <p className="text-sm text-slate-500">Fluxo principal</p>
          </div>
          {coreNode?.children.map((node) => (
            <NodeCard key={node.id} node={node} color="border-emerald-500" />
          ))}
        </div>

        <div className="space-y-4">
          <h3 className="text-xl font-semibold text-slate-700 text-center">{moduleNode?.label ?? 'Modulos'}</h3>
