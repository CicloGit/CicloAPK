...
const VIEW_ICONS: Record<string, any> = {
    'dashboard': HomeIcon, 'financials': CurrencyDollarIcon, 'stock': CubeIcon, 'commercial': TagIcon, 'sales': ShoppingCartIcon, 'logistics': TruckIcon, 'propertyRegistration': MapIcon, 'reports': ChartBarIcon, 'fieldOperations': ClipboardListIcon, 'liveHandling': QrCodeIcon, 'management': BeakerIcon, 'workforce': UsersIcon, 'contracts': DocumentReportIcon, 'futureMarket': TrendingUpIcon, 'carbonMarket': LeafIcon, 'customInputRequest': FlaskIcon, 'aiAnalysis': SparklesIcon, 'integrations': PuzzleIcon,
    'architecture': DiagramIcon, 'dataDictionary': BookIcon, 'operations': TableIcon, 'flows': FlowIcon, 'systemConfig': DiagramIcon,
    'producerPortal': BriefcaseIcon, 'technicianPortal': BriefcaseIcon, 'investorPortal': BriefcaseIcon, 'supplierPortal': BriefcaseIcon, 'integratorPortal': BriefcaseIcon,
    'finance': CashIcon, 'legal': BriefcaseIcon
};
...
const Sidebar: React.FC = () => {
  ...
  const producerSubNav = sectorSettings.navigation
  ...
  const navSections = [ { title: "Geral", items: [ { id: 'dashboard', label: 'Painel', path: '/dashboard' } ] }, { title: "Sistema", items: [ { id: 'integrations', label: 'Integrações', path: '/integrations' }, { id: 'architecture', label: 'Arquitetura', path: '/architecture' } ] } ];
  ...
  const renderNavForRole = () => {
          ...
          return (
             <div>
                <button
                    ...
                    className={`w-full text-left flex items-center p-3 my-1 rounded-lg transition-colors duration-200 ${isProducerSubNavActive ? 'bg-slate-700 text-white' : 'hover:bg-slate-700 text-slate-300'}`}
                >
                    <BriefcaseIcon className="h-6 w-6 mr-3 flex-shrink-0" />
                    <span className="text-sm">Gestão da Produção</span>
      ...
      return navSections.map(section => (
        <div key={section.title}>
            ...
            <ul>
                {section.items.filter(item => isViewAllowed(item.id)).map(item => (
                    <li key={item.id}>
    ...
    <aside className="w-64 bg-slate-800 text-slate-200 flex flex-col h-full">
      ...
      <div className="p-4 border-t border-slate-700 bg-slate-900">
        ...
        <div className="flex items-center">
            ...
            <button onClick={handleLogout} title="Sair" className="ml-auto p-2 rounded-md hover:bg-slate-700 text-slate-400 hover:text-white transition-colors">
