...
async function run() {
  ...
  const legacyMap: Record<string, { supplierItemId: string; supplierId: string }> = {};
  ...
  for (const reqDoc of requestsSnap.docs) {
    ...
    const newLines = lines.map((l: any) => {
      if (l.supplierId && l.supplierItemId) return l;
      changed = true;
    ...
    if (changed) {
      if (!DRY_RUN) await reqDoc.ref.update({ lines: newLines, legacyMigrated: true });
      report.requestsMigrated++;
    }
    if (newLines.some((l: any) => !l.supplierId || !l.supplierItemId)) report.legacyRequests++;
  }
